...
broker="openHABianpi"
topics = [("bedroom/light", 2), ("bedroom/moodLights", 2), ("bedoroom/temp", 0), ("bedroom/humidity", 0)]
...
def parseColor(msg):
    ...
    if (isHex >= 0):    # using html hexadecimal notation
        ...
        for i in range(3):
            if i < len(msg):
                if not i: hsv[i] = float(msg[i]) / 360.0
                else: hsv[i] = float(msg[i]) / 100.0
            else: hsv[i] = 0 # string was malformed
        ...
        for i in range(3):
            if i < len(msg):
                rgb[i] = int(msg[i])
            else: rgb[i] = 0 # string was malformed
        hsv = hsv_to_rgb(rgb[0] / 255.0, rgb[1] / 255.0, rgb[2] / 255.0)
...
def isON(msg):
    if msg.upper() == 'ON': return True
    elif msg == '1': return True
    else: return False
